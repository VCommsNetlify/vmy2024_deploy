...
const timer = setInterval(function() {
    ...
    document.getElementById("countdown").innerHTML = `
        <div>${padNumbers(days)}<span class="timer">ДНЕЙ</span></div>
        <div>${padNumbers(hours)}<span class="timer">ЧАСОВ</span></div>
        <div>${padNumbers(minutes)}<span class="timer">МИНУТ</span></div>
        <div>${padNumbers(seconds)}<span class="timer">СЕКУНД</span></div>
    `;

    if (distance < 0) {
        clearInterval(timer);
        document.getElementById("countdown").innerHTML = "<p>ДОБРО ПОЖАЛОВАТЬ ДОМОЙ!</p>";
    }
}, 1000);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    viewMoreBtn.addEventListener('click', function() {
        ...
        this.textContent = this.textContent === 'СМОТРЕТЬ МЕНЬШЕ' ? 'ПОСМОТРЕТЬ БОЛЬШЕ' : 'СМОТРЕТЬ МЕНЬШЕ';
    });
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    viewMoreButton.addEventListener("click", function() {
        ...
        if (viewMoreButton.textContent === "ПОСМОТРЕТЬ ДРУГИЕ ИСТОРИИ") {
            viewMoreButton.textContent = "ПОСМОТРЕТЬ МЕНЬШЕ ИСТОРИЙ";
        } else {
            viewMoreButton.textContent = "ПОСМОТРЕТЬ ДРУГИЕ ИСТОРИИ";
        }
    });
});
...
function createThumbnails() {
    ...
    for (var h = 1; h <= totalPages; h++) {
     ...
     if (h === 1) {
            thumbnail.src = 'Assets/3. Events, Gallery and Socials/3. News/05272024 may VMY2024 dailies founders DSUE D2V1 horizontal.png';
		} else if (h === 2) {
					thumbnail.src = 'Assets/3. Events, Gallery and Socials/3. News/05252024-v-malaysia-2024-day-2-learn-to-get-up-again-by-dato-sri-vijay-eswaran.png';
		} else if (h === 3) {
					thumbnail.src = 'Assets/3. Events, Gallery and Socials/3. News/05242024-v-malaysia-2024-day-4-making-the-wait-worthwhile-by-joseph-sriman-japadas-bismark.png';
		} else if (h === 4) {
					thumbnail.src = 'Assets/3. Events, Gallery and Socials/3. News/05232024-v-malaysia-2024-day-2-pay-the-price-of-hard-work-by-dato-sri-vijay-eswaran0d2d7a23-6719-4940-8bd8-ccd1398755f5.png';
		} else if (h === 5) {
					thumbnail.src = 'Assets/3. Events, Gallery and Socials/3. News/05222024-vmalaysia2024-highlights-day-5-unbelievable-unforgettable-unstoppableba1ce342-76af-4a38-9f6a-34053e400501.png';			
		} else if (h === 6) {
