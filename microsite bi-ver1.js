...
const timer = setInterval(function() {
    ...
    document.getElementById("countdown").innerHTML = `
        <div>${padNumbers(days)}<span class="timer">HARI</span></div>
        <div>${padNumbers(hours)}<span class="timer">JAM</span></div>
        <div>${padNumbers(minutes)}<span class="timer">MENIK</span></div>
        <div>${padNumbers(seconds)}<span class="timer">DETIK</span></div>
    `;

    if (distance < 0) {
        clearInterval(timer);
        document.getElementById("countdown").innerHTML = "<p>SELAMAT DATANG!</p>";
    }
}, 1000);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    viewMoreBtn.addEventListener('click', function() {
        ...
        this.textContent = this.textContent === 'LIHAT LEBIH SEDIKIT' ? 'LIHAT LAINNYA' : 'LIHAT LEBIH SEDIKIT';
    });
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    viewMoreButton.addEventListener("click", function() {
        ...
        if (viewMoreButton.textContent === "Lihat Cerita Lainnya") {
            viewMoreButton.textContent = "Lihat Lebih Sedikit Cerita";
        } else {
            viewMoreButton.textContent = "Lihat Cerita Lainnya";
        }
    });
});
