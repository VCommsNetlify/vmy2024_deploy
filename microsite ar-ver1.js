...
const timer = setInterval(function() {
    ...
    document.getElementById("countdown").innerHTML = `
        <div>${padNumbers(days)}<span class="timer">أيام</span></div>
        <div>${padNumbers(hours)}<span class="timer">ساعات</span></div>
        <div>${padNumbers(minutes)}<span class="timer">دقائق</span></div>
        <div>${padNumbers(seconds)}<span class="timer">ثواني</span></div>
    `;

    if (distance < 0) {
        clearInterval(timer);
        document.getElementById("countdown").innerHTML = "<p>مرحبًا بكم في الديار</p>";
    }
}, 1000);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    viewMoreButton.addEventListener("click", function() {
        ...
        if (viewMoreButton.textContent === "عرض المزيد من القصص") {
            viewMoreButton.textContent = "عرض قصص أقل";
        } else {
            viewMoreButton.textContent = "عرض المزيد من القصص";
        }
    });
});
